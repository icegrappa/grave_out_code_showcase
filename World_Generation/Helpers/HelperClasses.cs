...
public static class NativeTextureHelper {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static float GetLinearTexturePixel(int x, int y, ref NativeSlice<ushort> heightMap, int heightMapSize)
    {
        // Ensure the coordinates are within the bounds
        if (x < 0 || x >= heightMapSize || y < 0 || y >= heightMapSize)
        {
            return 0f; // or handle it as needed
        }

        // Correct indexing to row-major order
        var index = y * heightMapSize + x;

        return (float)heightMap[index] / ushort.MaxValue;
    }
    
    public static Color32 SampleTextureBilinear(ref NativeSlice<Color32> biomeMap, int biomeMapSize, float2 texturePos)
    {
        texturePos *= biomeMapSize;
        var x1 = (int)math.floor(texturePos.x);
        var y1 = (int)math.floor(texturePos.y);
        var x2 = math.clamp(x1 + 1, 0, biomeMapSize - 1);
        var y2 = math.clamp(y1 + 1, 0, biomeMapSize - 1);

        var xp = texturePos.x - x1;
        var yp = texturePos.y - y1;

        var p11 = GetLinearTexturePixel(x1, y1, ref biomeMap, biomeMapSize);
        var p12 = GetLinearTexturePixel(x2, y1, ref biomeMap, biomeMapSize);
        var p21 = GetLinearTexturePixel(x1, y2, ref biomeMap, biomeMapSize);
        var p22 = GetLinearTexturePixel(x2, y2, ref biomeMap, biomeMapSize);

        var px1 = LerpColor(p11, p12, xp);
        var px2 = LerpColor(p21, p22, xp);

        return LerpColor(px1, px2, yp);
    }
    
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Color32 GetLinearTexturePixel(int x, int y, ref NativeSlice<Color32> texture, int textureSize)
    {
        // Ensure the coordinates are within the bounds
        if (x < 0 || x >= textureSize || y < 0 || y >= textureSize)
        {
            return new Color32(0, 0, 0, 0); // or handle it as needed
        }
    ...
    private static Color32 LerpColor(Color32 c1, Color32 c2, float t)
    {
        return new Color32(
            (byte)math.lerp(c1.r, c2.r, t),
            (byte)math.lerp(c1.g, c2.g, t),
            (byte)math.lerp(c1.b, c2.b, t),
            (byte)math.lerp(c1.a, c2.a, t));
    }
