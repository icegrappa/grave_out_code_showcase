...
public struct PatchTriangulationJob : IJobParallelFor {
    ...
    [ReadOnly] public  NativeParallelHashMap<int2,int> vertexReferences;
    ...
    public void Execute(int patchIndex) {
        ...
        var patchVertexCountPerLine = (settings.TotalVertexCount - 1) / settings.CorePointsSpacing;
        var patchPosition = LinearArrayHelper.ReverseLinearIndex(patchIndex, patchVertexCountPerLine);

        var patchLineVertVertexCount = (settings.TotalVertexCount - 1) / patchVertexCountPerLine;
